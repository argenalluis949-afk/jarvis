...
# Cargar variables de entorno desde el archivo .env (entorno local)
load_dotenv()

app = FastAPI(title="J.A.R.V.I.S. HUD Interface")

# Inicialización del cliente con la API Key de Groq
        ...
        <script>
            ...
            async function iniciarSistema() {
                ...
                try {
                    ...
                    document.getElementById('responseText').innerText = "Sistemas de audio calibrados...";
            ...
            function iniciarSensorAplausos() {
                ...
                function medir() {
                    if (sistemaConectado && !jarvisEncendido && !estaHablando) {
                        ...
                        if (promedio > 55) {
                            let ahora = Date.now();
                            if (ahora - ultimoPico < 600 && ahora - ultimoPico > 100) {
                                encenderJarvis();
                            }
                            ultimoPico = ahora;
                        }
            ...
            function iniciarReconocimientoVoz() {
                ...
                if (!SpeechRecognition) {
                    document.getElementById('responseText').innerText = "Requisito de navegador: Google Chrome.";
                    return;
                }
                ...
                reconocedorVoz.onresult = async (event) => {
                    ...
                    try {
                        ...
                        hablar("Falla temporal de conexión.");
                    }
                };
...
async def procesar(data: EntradaTexto):
    if not client_groq:
        return {"respuesta": "Clave GROQ_API_KEY no configurada en las variables de entorno."}
    ...
    try:
        completion = client_groq.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Eres J.A.R.V.I.S., la inteligencia artificial avanzada de Tony Stark. Responde de forma concisa (máximo 2 oraciones), extremadamente elegante, educada y profesional en español."
                },
                {
                    "role": "user",
                    "content": data.texto
        ...
        return {"respuesta": f"Anomalía en el procesamiento central: {str(e)}"}
